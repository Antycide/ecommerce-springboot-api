...
public class ReviewControllerIntegrationTest {
    ...
    @Test
    void addAndGetReviews_flowWorks() {
        Product product = createProduct("Book");
        String jwt = loginAndGetJwt("test123", "test123");

        ResponseEntity<ShowReviewDto> createResponse = testRestTemplate.exchange(
                "/api/products/" + product.getId() + "/reviews",
                HttpMethod.POST,
                new HttpEntity<>(new AddReviewDto("Great"), bearerHeaders(jwt)),
                ShowReviewDto.class
        );

        assertThat(createResponse.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        assertThat(createResponse.getBody()).isNotNull();
        assertThat(createResponse.getBody().reviewText()).isEqualTo("Great");

        ResponseEntity<Map<String, Object>> getResponse = testRestTemplate.exchange(
                "/api/products/" + product.getId() + "/reviews?page=0&size=10",
                HttpMethod.GET,
        ...
        assertThat(((List<?>) getResponse.getBody().get("content"))).isNotEmpty();
    }
